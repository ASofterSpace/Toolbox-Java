...
public class PdfFile extends BinaryFile {
	...
	// by default, we use the xrefs that we read when we save the file again
	// however, if we build a file from scratch (after calling createEmptyDocument()),
	// then we want to actually build our own xrefs directly when saving
	private boolean buildXRefsOnSave = false;
	...
	public void createEmptyDocument() {
		...
		this.builderTexts = new ArrayList<>();
		this.builderPages = new ArrayList<>();
	...
	// returns the newline character used for writing (!) pdf
	// when reading, everything is incredibly chaotic anyway, as no matter what the standard says,
	// ALL kinds of line endings and even chaotic combinations exist in the wild xD
	private static String writeNL() {
		return "\n";
	}
	...
	public void save() {
		...
		if (buildXRefsOnSave) {
			xrefs = new ArrayList<>();
			xrefs.add("0 " + (objects.size()+1));
			xrefs.add("0000000000 65535 f");
		}

		for (PdfObject obj : objects) {
			if (buildXRefsOnSave) {
				String xrefstart = ""+pdf.length();
				String xrefzeroed = "0000000000";
				xrefs.add(xrefzeroed.substring(xrefstart.length()) + xrefstart + " 00000 n");
			}
			obj.appendToPdfFile(pdf, nl);
			pdf.append(nl);
		}
		...
		pdf.append("trailer");
		pdf.append(nl);
		trailer.appendToPdfFile(pdf, nl);
